...
/// Tests getting viewable category IDs when user has view permission.
///
/// Verifies that the repository returns category IDs where the user has a role
/// with can_view permission set to true.
///
/// Expected: Ok with category ID in list
#[tokio::test]
async fn returns_category_ids_with_view_permission() -> Result<(), DbErr> {
    ...
    let result = repo
        .get_viewable_category_ids_by_user(
...
/// Tests empty result when user has no view permission.
///
/// Verifies that the repository returns an empty list when the user has roles
/// but none have can_view permission.
///
/// Expected: Ok with empty vec
#[tokio::test]
async fn returns_empty_when_user_lacks_view_permission() -> Result<(), DbErr> {
    let test = TestBuilder::new()
        .with_fleet_tables()
        .build()
        .await
...
/// Tests multiple viewable categories.
///
...
#[tokio::test]
async fn returns_multiple_viewable_category_ids() -> Result<(), DbErr> {
    let test = TestBuilder::new()
        .with_fleet_tables()
        .build()
        .await
    ...
    // Category 1 with view permission
    ...
    // Category 2 with view permission
    ...
    // Category 3 without view permission (should be excluded)
    category_repo
        .create(CreateFleetCategoryParams {
            guild_id: guild.guild_id.parse().unwrap(),
            ping_format_id: ping_format.id,
...
/// Tests user with multiple roles having view access.
///
/// Verifies that the repository returns category IDs when the user has multiple
/// roles and at least one has view permission.
///
