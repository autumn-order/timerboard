...
/// Tests getting creatable category IDs when user has create permission.
///
...
#[tokio::test]
async fn returns_category_ids_with_create_permission() -> Result<(), DbErr> {
    ...
    let result = repo
        .get_creatable_category_ids_by_user(
...
/// Tests empty result when user has no create permission.
///
/// Verifies that the repository returns an empty list when the user has roles
/// but none have can_create permission.
///
/// Expected: Ok with empty vec
#[tokio::test]
async fn returns_empty_when_user_lacks_create_permission() -> Result<(), DbErr> {
    let test = TestBuilder::new()
        .with_fleet_tables()
        .build()
        .await
...
/// Tests multiple creatable categories.
///
...
#[tokio::test]
async fn returns_multiple_creatable_category_ids() -> Result<(), DbErr> {
    let test = TestBuilder::new()
        .with_fleet_tables()
        .build()
        .await
    ...
    // Category 1 with create permission
    ...
    // Category 2 with create permission
    ...
    // Category 3 without create permission (should be excluded)
    category_repo
        .create(CreateFleetCategoryParams {
            guild_id: guild.guild_id.parse().unwrap(),
            ping_format_id: ping_format.id,
...
/// Tests user with multiple roles having create access.
///
/// Verifies that the repository returns category IDs when the user has multiple
/// roles and at least one has create permission.
///
